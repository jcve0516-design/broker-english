...
(() => {
  ...
  function pickUSVoice() {
    ...
    // Prefer the most natural US voices — Ava first, then Samantha, then other neural
    // voices; reward Premium/Enhanced variants and penalize the robotic "compact" ones.
    const score = (v) => {
      const n = (v.name || "").toLowerCase();
      let s = 0;
      if (/\bava\b/.test(n)) s += 100;
      else if (/samantha/.test(n)) s += 60;
      else if (/\b(nathan|evan|zoe|allison|susan|tom|nicky|joelle|noelle)\b/.test(n)) s += 45;
      else if (/\b(alex|aaron|fred|google us english|aria|jenny|guy|zira|david)\b/.test(n)) s += 30;
  ...
  // Speak text phrase-by-phrase (sense groups + pauses) for natural, non-robotic
  // prosody. Shared by 口语范读 / 听写 / 短语库 / previews via the global voice + pause settings.
  // chunkSentence / pauseGaps / voiceFor are hoisted function declarations (safe at call time).
  function speak(text, rate) {
    if (!TTS || !text) return;
    try {
      TTS.cancel();
      const chunks = chunkSentence(String(text));
      const r = rate || state.settings.rate || 0.95;
      const vset = state.settings.voice;
  ...
  function initReading() {
    ...
    if (!sel) return;
    if (!RREAD.topics.length) {
      sel.innerHTML = "<option>（语料未加载）</option>";
      const list = $("#rdList");
      if (list) list.innerHTML = `<div class="muted" style="padding:12px">阅读语料未加载成功（多半是更新时网络中断）。请在<b>联网</b>状态下刷新本页一次即可恢复，之后可离线使用。</div>`;
      return;
    }
    sel.innerHTML =
  ...
  const DL_PAGE = 15;
  const dlState = { mi: 0, inited: false, playingAll: false, stopAt: 0, topic: "__all__", page: 0 };
  ...
  function dlInitMeetings() {
    ...
    sel.addEventListener("change", (e) => { dlStopAudio(); dlState.mi = Number(e.target.value) || 0; dlState.topic = "__all__"; dlState.page = 0; renderDialogues(); });
    const tsel = $("#dlTopicSel");
    if (tsel) tsel.addEventListener("change", (e) => { dlState.topic = e.target.value; dlState.page = 0; renderDialogues(); });
  ...
  function dlFillTopics(m) {
    const tsel = $("#dlTopicSel"); if (!tsel) return;
    const counts = {};
    m.turns.forEach((t) => { if (t.tp) counts[t.tp] = (counts[t.tp] || 0) + 1; });
    const keys = Object.keys(counts);
    if (!keys.length) { tsel.style.display = "none"; return; }
    tsel.style.display = "";
    tsel.innerHTML = `<option value="__all__">全部话题（${m.turns.length}）</option>` +
      keys.map((k) => `<option value="${escapeHtml(k)}"${k === dlState.topic ? " selected" : ""}>${escapeHtml(k)}（${counts[k]}）</option>`).join("");
    if (dlState.topic !== "__all__" && !counts[dlState.topic]) dlState.topic = "__all__";
    tsel.value = dlState.topic;
  }
  function renderDialogues() {
    ...
    const cnt = $("#dlCount");
    if (!m) { list.innerHTML = `<div class="muted" style="padding:12px">对话内容正在制作中，稍后更新。</div>`; if (cnt) cnt.textContent = ""; $("#dlPager").innerHTML = ""; const ts = $("#dlTopicSel"); if (ts) ts.style.display = "none"; return; }
    dlFillTopics(m);
    const all = m.turns.map((t, gi) => ({ t, gi }));
    const items = dlState.topic === "__all__" ? all : all.filter((x) => x.t.tp === dlState.topic);
    const pages = Math.max(1, Math.ceil(items.length / DL_PAGE));
    if (dlState.page >= pages) dlState.page = 0;
    if (cnt) cnt.textContent = `${items.length} 句${dlState.topic === "__all__" ? "" : " · " + dlState.topic}`;
    const hideEn = $("#dlHideEn") && $("#dlHideEn").checked;
    const hideZh = $("#dlHideZh") && $("#dlHideZh").checked;
    const start = dlState.page * DL_PAGE;
    list.innerHTML = items.slice(start, start + DL_PAGE).map(({ t, gi }) => {
      ...
      const tag = t.tp ? `<span class="ph-tag">${escapeHtml(t.tp)}</span>` : "";
      return `<div class="dl-turn" data-i="${gi}">
        <div class="dl-sp">${escapeHtml(t.sp || "")}${tag}</div>
        <div class="dl-en${hideEn ? " blur" : ""}">${en}</div>
        ${zh ? `<div class="dl-zh${hideZh ? " blur" : ""}">${zh}</div>` : ""}
        ${raw}
        <div class="dl-acts">
          <button class="btn ghost small" data-dl-au="${gi}">▶️ 原音</button>
          <button class="btn ghost small" data-dl-say="${gi}">🔊 范读</button>
          <button class="btn ghost small" data-dl-add="${gi}">＋ 生词本</button>
        </div>
      </div>`;
    }).join("");
    $("#dlPager").innerHTML = pages > 1
      ? `<button class="btn ghost small" data-dlpg="prev" ${dlState.page === 0 ? "disabled" : ""}>← 上一页</button>` +
        `<span class="rd-pageinfo">${dlState.page + 1} / ${pages}</span>` +
        `<button class="btn ghost small" data-dlpg="next" ${dlState.page >= pages - 1 ? "disabled" : ""}>下一页 →</button>`
      : "";
  }
  ...
  const dlPagerEl = $("#dlPager");
  if (dlPagerEl) dlPagerEl.addEventListener("click", (e) => {
    const b = e.target.closest("[data-dlpg]"); if (!b) return;
    if (b.dataset.dlpg === "prev" && dlState.page > 0) dlState.page--;
    else if (b.dataset.dlpg === "next") dlState.page++;
    renderDialogues();
  });
